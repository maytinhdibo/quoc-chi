...
const register = async (req, res) => {
    const { name, email, password, academicTitleId, organizationId, optionUserRole, optionEditorRole } = req.body;
    try {
        ...
        await db.user.create({
            name, email, encrypted_password, academicTitleId, organizationId
        }).then(newuser => {
            user = newuser;
        }).catch(e => {
            throw new Error("Lỗi với dữ liệu nhập vào, vui lòng kiểm tra lại.");
        });
        if (optionUserRole == 1) { //admin
            console.log("admin");
...
const get = async (req, res) => {
    ...
    try {
        ...
        if (chapter) {
            ...
            WHERE c.id=`+ chapter, { type: db.sequelize.QueryTypes.SELECT });
...
const getInfo = async (req, res) => {
    ...
    try {
        ...
        if (!user) {
            ...
            book_roles = await Promise.all(book_roles.map(async (role) => {
            ...
            res.json(response.success({
                name, email, phone, academic_title, organization, book_roles
            }));
        }
    } catch (e) {
        res.json(response.fail(e.message));
