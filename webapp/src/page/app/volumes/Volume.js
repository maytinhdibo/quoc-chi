  ...
  render() {
    const columns = [
      ...
      },
      {
        ...
        Cell: props => (
          <Link
            to={"/dashboard/chapters/detail/" + props.original.id}
            className="number"
          >
            {props.original.name}
          </Link>
        ...
        <div className="qc-content qc-card">
          <div className="qc-card-header">
            <span className="qc-header-title">
              Thông tin quyển:{" "}
              {this.state.data.volume && this.state.data.volume.name}
            </span>
              ...
              <div className="tool-dropdown">
                <div className="content">
                  <ul class="qc-dropdown-ul">
                    <Link
                      to={`/dashboard/volumes/${this.props.match.params.id}/newchapter/`}
                    >
                      <li>
                        <span className="icon">
                    ...
                    <Link
                      to={`/dashboard/volumes/edit/${this.props.match.params.id}`}
                    >
                      <li>
                        <span className="icon">
          ...
          {this.state.loaded ? (
            <div className="qc-content">
              ...
              <p
                dangerouslySetInnerHTML={
                  this.state.data.volume && {
                    __html: this.state.data.volume.description
                  }
                }
              ></p>
              <hr />
              ...
              <p>
                {this.state.data.volumeAdmin &&
                  this.state.data.volumeAdmin.length > 0 &&
                  this.state.data.volumeAdmin
                    .map(object => {
                      return (
                        <Link to={"/dashboard/user/" + object.user.id}>
                          {object.user.name}
                        </Link>
                      );
                    })
