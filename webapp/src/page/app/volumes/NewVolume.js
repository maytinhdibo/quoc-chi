...
class NewVolume extends React.Component {
    state = {
        actorValue: null,
        users: [],
        adminValue: [],
    ...
    componentDidMount() {
        ...
        analyticsAPI.getBook(this.props.match.params.id).then(object=>{
            if(object.success){
                this.setState({bookname:object.data.book.name})
            }
        })
    }
    ...
    newVolume = () => {
        ...
        volumeAPI.createVolume({ description, name, adminValue }, this.props.match.params.id).then(object => {
            if (object.success) {
                alertText("Tạo quyển thành công.");
            } else {
                alertText(object.reason);
            }
        }).catch(e => {
        ...
        return (
            <div className="qc-content qc-card">
                <div className="qc-card-header">
                   Tạo quyển mới trong tập:&nbsp;<b>{this.state.bookname}</b>
                    </div>
                <div className="qc-content">
                    ...
                    <FormGroup>
                        <Label>Danh sách trưởng quyển</Label>
                        <Select
                            isMulti
                            styles={inputStyle}
                ...
                <div className="qc-align-right qc-content">
                    <button onClick={this.newVolume} class="qc-btn">
