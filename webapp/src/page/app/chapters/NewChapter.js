...
class NewChapter extends React.Component {
    ...
    componentDidMount() {
        analyticsAPI.getVolume(this.props.match.params.id).then(object=>{
            if(object.success){
                this.setState({volumename:object.data.volume.name})
            }
        })
    }
    ...
    newChapter = () => {
        ...
        chapterAPI.createChapter({ description, name }, this.props.match.params.id).then(object => {
            if (object.success) {
                alertText("Tạo chương thành công.");
            } else {
        ...
        return (
            <div className="qc-content qc-card">
                <div className="qc-card-header">
                   Tạo chương mới trong quyển:&nbsp;<b>{this.state.volumename}</b>
                    </div>
                <div className="qc-content">
                    <FormGroup>
                        <Label for="exampleEmail">Tên chương</Label>
                        <Input value={this.state.name} onChange={(evt) => this.setState({ name: evt.target.value })} type="text" placeholder="Nhập tên chương mới" />
                    </FormGroup>
                ...
                <div className="qc-align-right qc-content">
                    <button onClick={this.newChapter} class="qc-btn">
